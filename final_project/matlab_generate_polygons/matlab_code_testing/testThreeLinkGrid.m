...
obs1(:,1)=data.obstacles.obstacle1x(1:end-1);
obs1(:,2)=data.obstacles.obstacle1y(1:end-1);
obs2=zeros(length( data.obstacles.obstacle2x(1:end-1) ), 2);
obs2(:,1)=data.obstacles.obstacle2x(1:end-1);
obs2(:,2)=data.obstacles.obstacle2y(1:end-1);

ObstacleList = {obs1, obs2};

link1Poly=zeros(length(data.links.linkPolys.link1x(1:end-1)),2);
link2Poly=zeros(length(data.links.linkPolys.link2x(1:end-1)),2);
link3Poly=zeros(length(data.links.linkPolys.link3x(1:end-1)),2);

link1Poly(:,1)=data.links.linkPolys.link1x(1:end-1);
link1Poly(:,2)=data.links.linkPolys.link1y(1:end-1);
link2Poly(:,1)=data.links.linkPolys.link2x(1:end-1);
link2Poly(:,2)=data.links.linkPolys.link2y(1:end-1);
link3Poly(:,1)=data.links.linkPolys.link3x(1:end-1);
link3Poly(:,2)=data.links.linkPolys.link3y(1:end-1);

link1Base=data.links.linkBases.base1;
link2Base=data.links.linkBases.base2;
link3Base=data.links.linkBases.base3;

fill(ObstacleList{1}(:,1),ObstacleList{1}(:,2), 'g' );
hold on
fill(ObstacleList{2}(:,1),ObstacleList{2}(:,2), 'k' );
%fill(ObstacleList{3}(:,1),ObstacleList{3}(:,2), 'k' );
axis([-3,3,-3,3]);
